...
subnat <- function () {  
  #getting regions, mortality collection, and name matching csv
  setwd("C:/Users/astut/Desktop/appui/data")
  domcode <- read.csv("domicile_code_table.csv", header = TRUE)
  #mort <- read.csv("mos3371.csv", header = TRUE)
  ...
  #dommort <- mort[ , c(13, 14, 20, 15:17, 66, 9, 31, 68:70, 32, 34)]#do indexing here
  
  #length(unique(domcode$TLA))
  #head(dommort)
...
#aagg = aggregate(DOD~GENDER+year+ageband,data = adf, FUN = function(x){NROW(x)})
aaggt = aggregate(DOD~GENDER+year+ageband+tlatext,data = adf, FUN = function(x){NROW(x)})
aaggd = aggregate(DOD~GENDER+year+ageband+dhbname,data = adf, FUN = function(x){NROW(x)})
aaggr = aggregate(DOD~GENDER+year+ageband+reg,data = adf, FUN = function(x){NROW(x)})

#aaggdep = aggregate(DOD~GENDER+year+ageband+Dep06,data = adf, FUN = function(x){NROW(x)})
aaggtdep = aggregate(DOD~GENDER+year+ageband+tlatext+Dep06,data = adf, FUN = function(x){NROW(x)})
aaggddep = aggregate(DOD~GENDER+year+ageband+dhbname+Dep06,data = adf, FUN = function(x){NROW(x)})
aaggrdep = aggregate(DOD~GENDER+year+ageband+reg+Dep06,data = adf, FUN = function(x){NROW(x)})


#magg = aggregate(DOD~GENDER+year+ageband,data = mdf, FUN = function(x){NROW(x)})
maggt = aggregate(DOD~GENDER+year+ageband + tlatext,data = mdf, FUN = function(x){NROW(x)})
maggd = aggregate(DOD~GENDER+year+ageband + dhbname,data = mdf, FUN = function(x){NROW(x)})
maggr = aggregate(DOD~GENDER+year+ageband + reg,data = mdf, FUN = function(x){NROW(x)})

#maggdep = aggregate(DOD~GENDER+year+ageband+Dep06,data = mdf, FUN = function(x){NROW(x)})
maggtdep = aggregate(DOD~GENDER+year+ageband + tlatext+Dep06,data = mdf, FUN = function(x){NROW(x)})
maggddep = aggregate(DOD~GENDER+year+ageband + dhbname+Dep06,data = mdf, FUN = function(x){NROW(x)})
maggrdep = aggregate(DOD~GENDER+year+ageband + reg+Dep06,data = mdf, FUN = function(x){NROW(x)})


#nmagg = aggregate(DOD~GENDER+year+ageband,data = nmdf, FUN = function(x){NROW(x)})
nmaggt = aggregate(DOD~GENDER+year+ageband + tlatext,data = nmdf, FUN = function(x){NROW(x)})
nmaggd = aggregate(DOD~GENDER+year+ageband + dhbname,data = nmdf, FUN = function(x){NROW(x)})
nmaggr = aggregate(DOD~GENDER+year+ageband + reg,data = nmdf, FUN = function(x){NROW(x)})

#nmaggdep = aggregate(DOD~GENDER+year+ageband+Dep06,data = nmdf, FUN = function(x){NROW(x)})
nmaggtdep = aggregate(DOD~GENDER+year+ageband + tlatext+Dep06,data = nmdf, FUN = function(x){NROW(x)})
nmaggddep = aggregate(DOD~GENDER+year+ageband + dhbname+Dep06,data = nmdf, FUN = function(x){NROW(x)})
nmaggrdep = aggregate(DOD~GENDER+year+ageband + reg+Dep06,data = nmdf, FUN = function(x){NROW(x)})
...
#other set up to help csvcreator function
...
setwd('C:/Users/astut/Desktop/appui/data/numerator')

csvcreator(aaggr, "total", "reg", regtext, 0)
csvcreator(aaggrdep, "total", "reg", regtext, 1)
csvcreator(aaggt, "total", "tla", tlatext, 0)
csvcreator(aaggtdep, "total", "tla", tlatext, 1)
csvcreator(aaggd, "total", "dhb", dhbtext, 0)
csvcreator(aaggddep, "total", "dhb", dhbtext, 1)

csvcreator(maggr, "maori", "reg", regtext, 0)
csvcreator(maggrdep, "maori", "reg", regtext, 1)
csvcreator(maggt, "maori", "tla", tlatext, 0)
csvcreator(maggtdep, "maori", "tla", tlatext, 1)
csvcreator(maggd, "maori", "dhb", dhbtext, 0)
csvcreator(maggddep, "maori", "dhb", dhbtext, 1)

csvcreator(nmaggr, "nonmaori", "reg", regtext, 0)
csvcreator(nmaggrdep, "nonmaori", "reg", regtext, 1)
csvcreator(nmaggt, "nonmaori", "tla", tlatext, 0)
csvcreator(nmaggtdep, "nonmaori", "tla", tlatext, 1)
csvcreator(nmaggd, "nonmaori", "dhb", dhbtext, 0)
csvcreator(nmaggddep, "nonmaori", "dhb", dhbtext, 1)

csvcreator <- function (givenagg,ethn,regi,subtext,dep) {
  ### Dropping the unknowns genders.
  agg = givenagg[givenagg$GENDER!="U",]
  
  if (dep == 0){
    colnames(agg) = c("G","Year","ageband",regi,"deaths")
  }else{
    colnames(agg) = c("G","Year","ageband",regi,"dep","deaths")
  }
  ...
  ##expand based on dep
  ryear = rep(seq(2006,2013,1), length(subtext))
  if (dep != 0) {
    ryear <- rep(ryear, 15)
  }
  ...
  newdf$reg <- rep(subtext, each = 24)
  
  if (dep != 0) {
    newdf$dep <- rep(c(1:10, 12, 34, 56, 78, 910), each = length(newdf$Year) / 15)
  }
  #fill empty frame
  ...
  for(i in 1:length(agg$Year)){
    if(dep == 0){
      if (regi == "reg") {
        ...
        newdf[which(newdf$Year == agg$Year[i]  & newdf$Factor == agg$Factor[i] & newdf$reg == agg$tla[i]),which(agg$ageband[i] == colnames(newdf))] = agg$deaths[i]        
      } else if (regi == "dhb") {
        newdf[which(newdf$Year == agg$Year[i]  & newdf$Factor == agg$Factor[i] & newdf$reg == agg$dhb[i]),which(agg$ageband[i] == colnames(newdf))] = agg$deaths[i]
      }
    }else{
      if(dep>10){
        if(dep>100){
        ...
        if (regi == "reg") {
          newdf[which(newdf$Year == agg$Year[i]  & newdf$Factor == agg$Factor[i] & newdf$reg == agg$reg[i] & d1 == agg$dep[i]),which(agg$ageband[i] == colnames(newdf))] = agg$deaths[i]
          newdf[which(newdf$Year == agg$Year[i]  & newdf$Factor == agg$Factor[i] & newdf$reg == agg$reg[i] & d2 == agg$dep[i]),which(agg$ageband[i] == colnames(newdf))] = newdf[which(newdf$Year == agg$Year[i]  & newdf$Factor == agg$Factor[i] & newdf$reg == agg$reg[i] & d2 == agg$dep[i]),which(agg$ageband[i] == colnames(newdf))] + agg$deaths[i]
        } else if (regi == "tla") {
          newdf[which(newdf$Year == agg$Year[i]  & newdf$Factor == agg$Factor[i] & newdf$reg == agg$tla[i] & d1 == agg$dep[i]),which(agg$ageband[i] == colnames(newdf))] = agg$deaths[i]
          newdf[which(newdf$Year == agg$Year[i]  & newdf$Factor == agg$Factor[i] & newdf$reg == agg$tla[i] & d2 == agg$dep[i]),which(agg$ageband[i] == colnames(newdf))] = newdf[which(newdf$Year == agg$Year[i]  & newdf$Factor == agg$Factor[i] & newdf$reg == agg$tla[i] & d2 == agg$dep[i]),which(agg$ageband[i] == colnames(newdf))] + agg$deaths[i]
        } else if (regi == "dhb") {
          newdf[which(newdf$Year == agg$Year[i]  & newdf$Factor == agg$Factor[i] & newdf$reg == agg$dhb[i] & d1 == agg$dep[i]),which(agg$ageband[i] == colnames(newdf))] = agg$deaths[i]
          newdf[which(newdf$Year == agg$Year[i]  & newdf$Factor == agg$Factor[i] & newdf$reg == agg$dhb[i] & d2 == agg$dep[i]),which(agg$ageband[i] == colnames(newdf))] = newdf[which(newdf$Year == agg$Year[i]  & newdf$Factor == agg$Factor[i] & newdf$reg == agg$dhb[i] & d2 == agg$dep[i]),which(agg$ageband[i] == colnames(newdf))] + agg$deaths[i]
        }
      }else{
        if (regi == "reg") {
          newdf[which(newdf$Year == agg$Year[i]  & newdf$Factor == agg$Factor[i] & newdf$reg == agg$reg[i] & newdf$dep == agg$dep[i]),which(agg$ageband[i] == colnames(newdf))] = agg$deaths[i]
        } else if (regi == "tla") {
          newdf[which(newdf$Year == agg$Year[i]  & newdf$Factor == agg$Factor[i] & newdf$reg == agg$tla[i] & newdf$dep == agg$dep[i]),which(agg$ageband[i] == colnames(newdf))] = agg$deaths[i]        
        } else if (regi == "dhb") {
          newdf[which(newdf$Year == agg$Year[i]  & newdf$Factor == agg$Factor[i] & newdf$reg == agg$dhb[i] & newdf$dep == agg$dep[i]),which(agg$ageband[i] == colnames(newdf))] = agg$deaths[i]
        }
      }
    }
  }
  ...
  if (dep != 0) {
    newdf[newdf$dep == 12, 3:19] <- newdf[newdf$dep == 1, 3:19] + newdf[newdf$dep == 2, 3:19]
    newdf[newdf$dep == 34, 3:19] <- newdf[newdf$dep == 3, 3:19] + newdf[newdf$dep == 4, 3:19]
    newdf[newdf$dep == 56, 3:19] <- newdf[newdf$dep == 5, 3:19] + newdf[newdf$dep == 6, 3:19]
    newdf[newdf$dep == 78, 3:19] <- newdf[newdf$dep == 7, 3:19] + newdf[newdf$dep == 8, 3:19]
    newdf[newdf$dep == 910, 3:19] <- newdf[newdf$dep == 9, 3:19] + newdf[newdf$dep == 10, 3:19]
  }
  ...
  write.csv(newdf,file=paste("mort",ethn, if (dep != 0) {"dep"} else {""},regi,".csv",sep=""),row.names=FALSE)
}
