...
shinyServer(function(input, output, session) {
  ...
  output$conditions <- renderUI({
    #check to determine what function is being used
    if (input$proj == FALSE){   
      ...
      tabsetPanel(
        ...
        tabPanel("Summary",                   
                 selectInput('format', "Select a format", c("by pop","by statistic"), 
                 selected = "by pop"), tableOutput("summary")
        ),
...
#   }
  ...
  getData <- function(popn, ethn, issue, prior, dep, regi){
    if(issue == "B.Y.O."){
        ...
        if(dep != 0 & regi == "National"){
        ...
        }else if(dep != 0 & regi == "Regional Council"){
          ...
          outputdata <- read.csv(toString(csvs[which(csvs$ethn == ethn & csvs$issue == issue & csvs$dep == -1 & csvs$geog == "Regional Council" & csvs$country == "New Zealand"), 4]), sep = as.character(csvs[which(csvs$ethn == ethn & csvs$issue == issue & is.na(csvs$dep) & csvs$geog == "Regional Council" & csvs$country == "New Zealand"), 3]), header = TRUE)
          outputdata <- outputdata[which(outputdata$reg == input$reg), ]
          outputdata <- outputdata[which(outputdata$dep == dep), ]
          ...
          outputdata <- outputdata[which(outputdata$reg == input$reg), ]
        }else if(dep != 0 & regi == "TLA"){
          outputdata <- read.csv(toString(csvs[which(csvs$ethn == ethn & csvs$issue == issue & csvs$dep == -1 & csvs$geog == "TLA" & csvs$country == "New Zealand"), 4]), sep = as.character(csvs[which(csvs$ethn == ethn & csvs$issue == issue & is.na(csvs$dep) & csvs$geog == "TLA" & csvs$country == "New Zealand"), 3]), header = TRUE)
          outputdata <- outputdata[which(outputdata$reg == input$tla), ]
          outputdata <- outputdata[which(outputdata$dep == dep), ]
        }else if(regi == "TLA"){
          ...
          outputdata <- outputdata[which(outputdata$reg == input$tla), ]
        }else if(dep != 0 & regi == "DHB"){
          outputdata <- read.csv(toString(csvs[which(csvs$ethn == ethn & csvs$issue == issue & csvs$dep == -1 & csvs$geog == "DHB" & csvs$country == "New Zealand"), 4]), sep = as.character(csvs[which(csvs$ethn == ethn & csvs$issue == issue & is.na(csvs$dep) & csvs$geog == "DHB" & csvs$country == "New Zealand"), 3]), header = TRUE)
          outputdata <- outputdata[which(outputdata$reg == input$dhb), ]
          outputdata <- outputdata[which(outputdata$dep == dep), ]
        }else if(regi == "DHB"){
        ...
        if(is.na(deaths[r]) ){
          ...
          if(input$stdtype == "External"){
            ...
            if(plotdata$ageband[r] == "Total"){
                ...
                varasr <- varasr + 100000*(plotdata$percent[r]*100000/plotdata$people[r])*((as.numeric(data_standard[which(data_standard$Standard == input$extstd), (which(selectgrouptext == plotdata$ageband[r]))]))^2)
                ...
                plotdata$rategammalow[r] <- 1/2*qchisq(0.05/2, 2*plotdata$deaths[r])/plotdata$people[r]*100000
                plotdata$rategammaupp[r] <- 1/2*qchisq(1-0.05/2, 2*(plotdata$deaths[r]+1))/plotdata$people[r]*100000
                plotdata$rateupp[r] <- plotdata$rate[r] + 1.96*(sqrt(plotdata$percent[r]/plotdata$people[r])*100000)  
...
#     plotdata$flagrr <- 0
    ...
    ardf$rateupp <- ardf$rate-arsub$rate + 1.96*sqrt(varar)*100000
    ardf$ratelow <- ardf$rate-arsub$rate - 1.96*sqrt(varar)*100000
  ...
  createProjections <- function (lastyear) {
    ...
    rates <- lastyear$rate[which(as.numeric(as.character(lastyear$year)) == max(as.numeric(as.character(lastyear$year))))]
    if (input$geog == "National") {
      place <- "nat"
    } else if (input$subgeog == "Regional Council") {
      place <- "reg"
    ...
    if (input$agegroup == "Total") {
      ...
      fixSelect <- input$selectgroups
      fixSelect <- gsub("X0.4", "00-04", fixSelect)
      fixSelect <- gsub("X5.9", "05-09", fixSelect)
      fixSelect <- gsub("X", "", fixSelect)
      fixSelect <- gsub("[.]", "-", fixSelect)
      fixSelect <- gsub("80-and-over", "80+", fixSelect)
      meltdata <- meltdata[which(is.element(meltdata$ageband, fixSelect)), ]
    }   
    
    meltdata$Population <- as.character(meltdata$Population)
    for(i in 1:length(meltdata$Population)){
      if(meltdata$Population[i] == "1"){
        if(input$gender1 == "Total"){
          if(input$ethnic1 == "Total"){
            pop1 <- "Total Pop 1"
          }else{
        ...
        meltdata$Population[i] <- pop1
        pop1 <- NULL
      }else if(meltdata$Population[i] == "2"){
        if(input$gender2 == "Total"){
          if(input$ethnic2 == "Total"){
            pop2 <- "Total Pop 2"
          }else{
        ...
        meltdata$Population[i] <- pop2
        pop2 <- NULL
      }else if(meltdata$Population[i] == "3"){
        if(input$gender3 == "Total"){
          if(input$ethnic3 == "Total"){
            pop3 <- "Total Pop 3"
          }else{
        ...
        meltdata$Population[i] <- pop3
        pop3 <- NULL
      }else if(meltdata$Population[i] == "4"){
        if(input$gender4 == "Total"){
          if(input$ethnic4 == "Total"){
            pop4 <- "Total Pop 4"
          }else{
        ...
        meltdata$Population[i] <- pop4
        pop4 <- NULL
      }
    }
    meltdata$Population <- as.factor(meltdata$Population)
    meltdf <<- meltdata
    meltdata[-which(is.element(meltdata$Year, c(1948:2012, 2013))), ]
  }
